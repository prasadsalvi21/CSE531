...
public class MST_Prasad_Salvi_prasadde {
	public static int n = 0, m = 0, u = 0, v = 0, w = 0, temp = 0, sum = 0,
			s = 0, i = 0;
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (n > 10000 || m > 100000 || n < 1 || m < 1) {
				...
				if (nl != m) {
					System.out
							.println("The number of entries for edges in file "
									+ nl + " doesnot match with m=" + m
									+ " specified");
				}
				for (i = 0; i < g.size(); i++) {
				...
				for (int i8 = 2; i8 <= n; i8++) {
					System.out.println(parent[i8] + "\t" + child[i8] + "\t"
							+ key[i8]);
					sum = sum + key[i8];
				}
